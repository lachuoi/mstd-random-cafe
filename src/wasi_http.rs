...
async fn http_request_recursive(
    ...
    if let Some(ref b) = body {
        for chunk in b.chunks(4096) {
            stream.blocking_write_and_flush(chunk).map_err(|_| anyhow::anyhow!("failed to write body chunk"))?;
        }
    }
            ...
            if matches!(status, 301 | 302 | 303 | 307 | 308) {
                ...
                if !location_vec.is_empty() {
                    if let Some(location_bytes) = location_vec.get(0) {
                        let location = String::from_utf8_lossy(location_bytes).to_string();
                        ...
                        let is_get = matches!(method, bindings::http::types::Method::Get);
                        let next_method = if status == 303 || (matches!(status, 301 | 302) && !is_get) {
